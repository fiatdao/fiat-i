...
contract NotionalMinter is ERC1155Holder {
    ...
    constructor(address _notional, uint16 currencyId, uint40 maturity) {
    ...
    function mintFromAsset(uint256 fCashAmount, address receiver) external {
        (uint8 marketIndex, int256 assetCashInternal /* int256 underlyingCashInternal*/, ) = _calculateMint(
            fCashAmount
        );
    ...
    function _calculateMint(
        uint256 fCashAmount
    ) internal returns (uint8 marketIndex, int256 assetCashInternal, int256 underlyingCashInternal) {
    ...
    function _getUnderlyingToken(
        uint16 currencyId
    ) private view returns (IERC20 underlyingToken, int256 underlyingPrecision) {
