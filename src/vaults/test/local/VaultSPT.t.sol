...
contract VaultSPT_Test is Test {
    ...
    function setUp() public {
        ...
        vault = IVault(
            vaultFactory.createVault(address(impl), abi.encode(maturity, address(sP_cUSDC), address(collybus)))
        );
        vault18 = IVault(
            vaultFactory.createVault(address(impl18), abi.encode(maturity, address(sP_cDAI), address(collybus)))
        );
