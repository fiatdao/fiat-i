...
import {IVaultFCv2} from "./interfaces/IVaultFCv2.sol";
...
/// @title VaultFCv2 (Notional fCash Vault)
/// @notice Collateral adapter for Notional fCash
/// @dev
contract VaultFCv2 is Guarded, IVaultFCv2, ERC1155Holder {
    ...
    /// @notice Notional Finance Maximum supported tenor [seconds]
    uint256 public immutable maxTenor;
    ...
    function enter(uint256 tokenId, address user, uint256 amount) external virtual override {
        ...
        if (!isValidTenor(tokenId)) revert VaultFC__enter_wrongTenor();
    ...
    function redeemAndExit(uint256 tokenId, address user, uint256 amount) external override returns (uint256 redeemed) {
        int256 wad = toInt256(wdiv(amount, tokenScale));
        if (currencyId != _getCurrencyId(tokenId)) revert VaultFC__exit_wrongCurrency();
        codex.modifyBalance(address(this), tokenId, msg.sender, -wad);
    ...
    function redeems(uint256 tokenId, uint256 amount, uint256 cTokenExRate) external view override returns (uint256) {
        INotionalV2.AssetRateParameters memory ar = INotionalV2(token).getSettlementRate(
            uint16(currencyId),
            uint40(maturity(tokenId))
        );
    ...
    /// @notice Returns the maturity for a given tokenId (fCashId)
    /// @param tokenId fCashId (ERC1155 token id)
    /// @return maturity [seconds]
    function maturity(uint256 tokenId) public pure override returns (uint256) {
        return uint40(tokenId >> 8);
    }

    /// @notice Validates that the tenor for a given tokenId (fCashId) is less or equal to `maxTenor`
    /// @param tokenId fCashId (ERC1155 token id)
    /// @return True if tenor is not greater than `maxTenor`
    function isValidTenor(uint256 tokenId) public view returns (bool) {
        return (maturity(tokenId) <= add(block.timestamp, maxTenor));
    }
    ...
    function fairPrice(uint256 tokenId, bool net, bool face) external view override returns (uint256) {
