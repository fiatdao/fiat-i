...
import {WAD, toInt256, add, sub, wmul, wdiv, mul, div} from "../utils/Math.sol";
...
contract VaultSY is Guarded, IVault, ERC165, ERC1155Supply, ERC721Holder {
    ...
    constructor(address codex_, address collybus_, address market_, string memory uri) Guarded() ERC1155(uri) {
        live = 1;
        codex = ICodex(codex_);
    ...
    function terms(uint256 bondId) public view returns (uint256 principal, uint256 maturity_, bool liquidated) {
    ...
    function unwrap(uint256 bondId, address to, uint256 amount) external {
