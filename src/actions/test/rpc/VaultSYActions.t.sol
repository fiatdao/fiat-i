...
contract VaultSYActions_RPC_tests is Test, ERC1155Holder, ERC721Holder {
    ...
    function _collateral(address vault_, uint256 tokenId, address user) internal view returns (uint256) {
        (uint256 collateral, ) = codex.positions(vault_, tokenId, user);
        return collateral;
    }

    function _normalDebt(address vault_, uint256 tokenId, address user) internal view returns (uint256) {
