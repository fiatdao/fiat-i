...
import {LeverSPTActions} from "../../lever/LeverSPTActions.sol";
...
contract LeverSPTActions_RPC_tests is Test {
    ...
    ) internal view returns (LeverSPTActions.SellFIATSwapParams memory fiatSwapParams) {
    ...
    function _getBuyFIATSwapParams(
        address assetIn,
        uint256 maxAmountIn
    ) internal view returns (LeverSPTActions.BuyFIATSwapParams memory fiatSwapParams) {
    ...
    function _getRedeemParams(
        address adapter
    ) internal view returns (LeverSPTActions.PTokenRedeemParams memory redeemParams) {
    ...
    function test_buyCollateralAndIncreaseLever() public {
        ...
        uint256 estDeltaCollateral = leverActions.underlierToPToken(address(maDAISpace), balancerVault, totalUnderlier);
