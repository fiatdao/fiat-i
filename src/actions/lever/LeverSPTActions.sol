...
interface IBalancerVault {
    ...
    function getPoolTokens(
        bytes32 poolId
    ) external view returns (ERC20[] memory tokens, uint256[] memory balances, uint256 maxBlockNumber);
}
...
contract LeverSPTActions is Lever20Actions, ICreditFlashBorrower, IERC3156FlashBorrower {
    ...
    function _onSellCollateralAndDecreaseLever(address initiator, uint256 borrowed, bytes memory data) internal {
    ...
    function _onRedeemCollateralAndDecreaseLever(address initiator, uint256 borrowed, bytes memory data) internal {
