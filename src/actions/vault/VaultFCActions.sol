...
interface INotional {
    ...
    function getCurrency(
        uint16 currencyId
    ) external view returns (Token memory assetToken, Token memory underlyingToken);
}
...
library EncodeDecode {
    ...
    function decodeERC1155Id(uint256 id) internal pure returns (uint16 currencyId, uint40 maturity, uint8 assetType) {
        assetType = uint8(id);
        maturity = uint40(id >> 8);
        currencyId = uint16(id >> 48);
    }

    function encodeERC1155Id(uint256 currencyId, uint256 maturity, uint256 assetType) internal pure returns (uint256) {
...
contract VaultFCActions is Vault1155Actions {
    ...
    /// @param maxImpliedRate Max. accepted annualized implied borrow rate for swapping fCash for underliers [1e9]
    function _sellfCash(uint256 tokenId, address to, uint256 fCashAmount, uint32 maxImpliedRate) internal {
    ...
    function exitVault(address vault, address token, uint256 tokenId, address to, uint256 amount) public override {
    ...
    function setApprovalForAll(address token, address spender, bool approved) external {
