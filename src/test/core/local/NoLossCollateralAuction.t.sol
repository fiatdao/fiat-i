...
import {WAD, sub, mul, wdiv} from "../../../core/utils/Math.sol";
...
contract Trader {
    ...
    constructor(
        NoLossCollateralAuction collateralAuction_,
        Codex codex_,
        DSToken gold_,
        Vault20 goldVault_,
        DSToken credit_,
...
contract Guy {
    ...
    constructor(NoLossCollateralAuction collateralAuction_) {
        collateralAuction = collateralAuction_;
    }
...
contract StartGuy is Guy {
    ...
    constructor(NoLossCollateralAuction collateralAuction_, address vault_) Guy(collateralAuction_) {
        vault = vault_;
    }
...
contract NoLossCollateralAuctionTest is Test {
    Codex codex;
    Limes limes;
    Collybus collybus;
    Aer aer;
    ...
    function test_globalMaxDebtOnAuction_maxDebtOnAuction() public {
        ...
        NoLossCollateralAuction collateralAuction2 = new NoLossCollateralAuction(address(codex), address(limes));
    ...
    function test_takeCollateral_over_debt() public takeCollateralSetup {
        ...
        // Readjusts collateralSlice to be collateralToSell
        ...
        assertEq(codex.balances(vault, tokenId, ali), 40 ether); // Took all collateral
        assertLt(codex.credit(ali), 890 ether); // Payed at least than debt (110)
        assertEq(codex.balances(vault, tokenId, me), 960 ether); // 960 + (40)
    ...
    function test_takeCollateral_partial_collateralToSell_full_debt() public takeCollateralSetup {
        assertEq(limes.globalDebtOnAuction(), 110 ether);

        Guy(ali).takeCollateral({
            auctionId: 1,
            collateralAmount: 39 ether,
            maxPrice: 5 ether,
            recipient: address(ali),
            data: ""
        });
        ...
        assertEq(limes.globalDebtOnAuction(), 0);
        assertEq(codex.balances(vault, tokenId, ali), 39 ether); // Took part of the collateral
        assertLt(codex.credit(ali), 890 ether); // Payed more than debt (110)
        assertEq(codex.balances(vault, tokenId, me), 961 ether); // 960 + (40)
    ...
    // partial debt not possible, since NoLossCollateralAuction enforces floor price of debt / collateral
    function test_takeCollateral_full_collateralToSell_partial_debt() public takeCollateralSetup {
        ...
        Guy(ali).takeCollateral({
            ...
            maxPrice: 3.1 ether, // price ~= 3.0
            recipient: address(ali),
            data: ""
        });

        assertEq(codex.balances(vault, tokenId, ali), 40 ether); // Took entire collateralToSell
        assertTrue(sub(codex.credit(ali), uint256(878.9 ether)) < 0.1 ether); // Paid at least 110 ether
    ...
    function test_takeCollateral_bid_avoids_recalculate_due_no_more_collateralToSell() public takeCollateralSetup {
        vm.warp(block.timestamp + 59); // Reducing the price
        ...
        assertEq(price, 2763417385811926650); // 2.73 WAD, 2735783211953807380973706855
    ...
    function test_takeCollateral_multiple_bids_different_prices() public takeCollateralSetup {
        ...
        assertEq(codex.balances(vault, tokenId, bob), _collateralToSell); // Didn't take whole collateralToSell
        assertLt(codex.credit(bob), 940 ether); // Paid rest of debt (more than 60)

        // uint256 collateralReturn = 30 ether - expectedToken; // collateralToSell - loaf.debt / maxPrice = 15
        assertEq(codex.balances(vault, tokenId, me), 960 ether); // Collateral returned (10 WAD)
    }
    ...
    function test_auction_reset_tail() public {
        ...
        (, , , , , , uint96 startsAtBefore, uint256 startPriceBefore) = collateralAuction.auctions(1);
        assertEq(uint256(startsAtBefore), startTime);
        ...
        (, , , , , , uint96 startsAtAfter, uint256 startPriceAfter) = collateralAuction.auctions(1);
        assertEq(uint256(startsAtAfter), startTime + 3601 seconds); // (block.timestamp)
        assertEq(startPriceAfter, 3.75 ether); // $3 collybus + 25% buffer = $5 (used most recent OSM price)
    }

    function test_auction_reset_floor_price() public {
        auctionResetSetup(1 hours); // 1 hour till zero is reached

        (, , , , , , uint96 startsAtBefore, uint256 startPriceBefore) = collateralAuction.auctions(1);
        assertEq(uint256(startsAtBefore), startTime);
        assertEq(startPriceBefore, 5 ether); // $4 collybus + 25% buffer = $5 (wasn't affected by spot update)

        // startPrice * ((duration - x) / duration) = floorPrice
        // 5 * ((3600 - x) / 3600) = 2.75
        vm.warp(startTime + 1619 seconds);
        (bool needsRedo, , , ) = collateralAuction.getStatus(1);
        assertTrue(!needsRedo);
        assertTrue(!try_redoAuction(1, address(this)));
        // vm.warp(startTime + 1801 seconds);
        vm.warp(startTime + 1620 seconds);
        (needsRedo, , , ) = collateralAuction.getStatus(1);
        assertTrue(needsRedo);
        collybus.updateSpot(address(gold), 3 ether); // Collybus = $1.50 (update price before reset is called)
        assertTrue(try_redoAuction(1, address(this)));
        ...
        (, , , , , , uint96 startsAtAfter, uint256 startPriceAfter) = collateralAuction.auctions(1);
        assertEq(uint256(startsAtAfter), startTime + 1620 seconds); // (block.timestamp)
    ...
    function test_auction_reset_floor_price_twice() public {
        auctionResetSetup(1 hours); // 1 hour till zero is reached (used to test maxDiscount)

        vm.warp(startTime + 1620 seconds); // Price goes below floor price after 51min45sec
        collateralAuction.redoAuction(1, address(this));
    ...
    function test_redoAuction_incentive() public takeCollateralSetup {
        ...
        assertEq(price, 2763417385811926650); // 2763417385811926650, 2.76 WAD
    ...
    function test_gas_full_takeCollateral() public takeCollateralSetup {
        ...
        assertEq(codex.balances(vault, tokenId, ali), 40 ether); // Took the whole collateral
        assertLt(codex.credit(ali), 890 ether); // Payed more than debt (110)
        assertEq(codex.balances(vault, tokenId, me), 960 ether); // 960 + (40)
