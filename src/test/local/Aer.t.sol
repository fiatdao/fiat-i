...
contract AerTest is DSTest {
    ...
    function test_unqueueDebt_auctionDelay() public {
        ...
        hevm.warp(queuedAt + 99 seconds);
        assertTrue(!try_unqueueDebt(queuedAt));
        hevm.warp(queuedAt + 100 seconds);
        assertTrue(try_unqueueDebt(queuedAt));
    }
    ...
    function test_transferCredit() public {
        hevm.store(address(codex), keccak256(abi.encode(address(aer), uint256(5))), bytes32(uint256(10 ether)));
